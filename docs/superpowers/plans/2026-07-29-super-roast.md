...
- Severity floors (verbatim list, used in Task 6 and SKILL.md): confirmed injection / authZ bypass / secrets-in-code **that is potentially exploitable to escalate privilege or reach data the invoker could not already reach** (any network-exposed surface qualifies; so does local tooling running with privileges its caller lacks — a flaw whose only "attacker" is the invoker feeding their own inputs, gaining nothing they did not already hold, is judged on its merits instead); data-loss or irreversible-migration risk on real data; violation of the artifact's own stated core purpose → **Blocking under any profile**. *(Amended 2026-07-30 after the PR-mode live run: all three seats declined to floor a local-CLI injection because argv was developer-supplied, which was a faithful reading of the previous "network-exposed surface" wording — human ruled the escalation framing is the right test.)*
...
- [ ] **Step 1: Write `reporter-prompt.md`** with these rules:
  ...
  - **Prior report handling:** mark each previous confirmed finding resolved/regressed/still-open; do not re-litigate its Rejected section. **Narrow exception (evidence-disciplined):** a previously-rejected finding may be reconsidered only when the current iteration's evidence differs materially from what the prior report cited — typically because the fixes changed the very code or spec text that rejection rested on — and the reconsideration must cite the specific new evidence, exactly as a panel overrule must. Absent that citation, the prior rejection stands. This exists because a fix can legitimately turn a previously-immaterial claim into a real one; it is not a licence to re-argue rejections.
...
- [ ] **Step 1: Write SKILL.md** following roast's structure (Overview → When to Use → Process → Output → Model Tiering → Red Flags → Limitations → Integration), updated:
  ...
  - Red Flags (port roast's, then update): never dispatch three identical judge prompts — one judge per seat; never let the profile demote a floors-class finding; never re-litigate a prior report's Rejected section except under the narrow evidence-disciplined exception (materially changed evidence, cited); never edit the artifact or create tasks (report-only); never guess the mode on ambiguous input; never report a clean verdict when scouts/judges failed — `[low coverage]`; never CONFIRM an external claim without a resolved citation; UNVERIFIED/incomplete panels escalate, never drop.
...
- [x] **Step 1 DONE (2026-07-30, run `wf_b5c99289-978`).** Pipeline ran end to end: 138 agents, 0 errors, 127/127 judge votes, report at `docs/superpowers/reviews/2026-07-30-depth-cap-spec-roast-1.md`, verdict `Blocking (34)`. Profile applied with stated justification; floors held; all report sections rendered. Findings from the run are addressed in Step 1b below.

- [ ] **Step 1b: Fix what the live run exposed** (human-approved 2026-07-30). Five items, then commit:
  1. **Dedupe liveness (Blocking, found by the skill in its own design).** `super-roast-workflow.md`: the dedupe agent has no retry and no liveness check, so a dead/truncated dedupe yields zero packets and the pipeline reports `clean (0 nits)` — contradicting the engine's own "dead agents ⇒ low coverage, never a silent clean" rule for the one stage the prompt file calls silent-and-unrecoverable. Retry dedupe once (same shape as `seat()`), add a `dedupeDead` boolean to the returned `coverage`, and force the low-coverage signal when `raw.length > 0 && deduped.length === 0`.
  2. **Coverage never reaches the reporter (Blocking, same source).** The reporter is required to emit the coverage line and the `[low coverage]` qualifier but is passed only `{{PACKETS_JSON}}`/`{{PROFILE}}`/`{{PRIOR_REPORT}}`, and `coverage` is assembled after it runs. Add a `{{COVERAGE_JSON}}` token: build the coverage object (minus reporter-dependent fields) *before* the reporter call, fill it into the prompt, and document the token in the workflow doc's prompt contract and in `reporter-prompt.md`.
  3. **Panel cap — restores the cost control (the run cost 7.78M tokens / 56 min).** `config.remainderCap` bounds only the Nit/FYI tail, so 51 deduped findings produced 38 full panels + 13 spot checks = 127 seat calls. Add `config.panelCap` (default **12**): after the deduper's ordering, the top-`panelCap` severe candidates get full panels; severe candidates beyond it are reported in a new **`## Not verified (beyond panel cap)`** section with their suggested severity and the count — listed, never dropped, matching roast's no-silent-drop principle. Add `beyondPanelCap` to `coverage`, require the reporter to render the section and to append `[panel-capped: N unverified]` to the verdict line when it is non-empty, and document `panelCap` in SKILL.md beside `remainderCap`.
  4. **Scope pinning (scouts reviewed the wrong artifact).** 26 of 51 findings were verified against the *successor* spec because scouts wandered to adjacent files in the same directory. In `scout-prompts-design.md` and `scout-prompts-pr.md`: review **only** the named artifact; you may read other files solely to understand it, and a finding whose evidence cites any other file is out of scope and must not be reported. Add the same one-line restriction to the shared core of `judge-seat-prompts.md` (a seat must not verify against a different artifact).
  5. **Verdict format.** The run emitted `Blocking (34)`; the spec's format is `<highest confirmed severity> (<n> confirmed)`. Tighten `reporter-prompt.md` so the count carries the word `confirmed`.

- [ ] **Step 1c: dryRun after the engine change** (items 1-3 are structural). Re-run the PR-mode topology dryRun with `panelCap: 1` and a dedupe stub returning 2 Blocking + 1 Nit: assert 1 full panel (3 seats), `beyondPanelCap === 1`, the second severe finding absent from panels, `dedupeDead === false`, and `{{COVERAGE_JSON}}` present in the reporter's rendered prompt. Then a second run with a dedupe stub returning nothing: assert `dedupeDead === true` and that the low-coverage signal fires. Record both in the workflow doc's assertions section.

- [ ] **Step 2: PR-mode live run** (one run, under the fixed config). Branch `super-roast-live-test` already exists at `a71e3a4` with the two planted defects in `scripts/collect-metrics.js` (SQL injection L39-40; unbounded module-level cache L36/L42 fed by `watchLoop` L66-77). Run super-roast in PR mode against `worktree-super-roast..super-roast-live-test`. Verify: mode classified `pr`; `security`/`correctness` lanes fired; **the injection is a confirmed Blocking** (floor applies regardless of the repo's hobby-grade profile); the unbounded cache is confirmed (severity may legitimately land Should-fix or Nit given the profile — that is the env-awareness working, record which); no confirmed finding about pre-existing repo code; cost materially below the design run's 7.78M tokens.

- [ ] **Step 3: Cleanup + record.** Delete the test branch. Commit both reports as eval evidence: `git commit -m "test(super-roast): live design-mode and PR-mode validation reports"`. If the PR run misbehaves structurally, fix the responsible file, dryRun if the engine changed, and re-run.
