...
**2026-06-13 — Implemented as designed.** Built skill-prose only under `skills/roast/` (`SKILL.md`, `roast-workflow.md`, `critic-prompt.md`, `judge-prompt.md`, `domain-triage-prompt.md`) plus an optional `roast` gate wired into `brainstorming` (checklist item, process-flow node, and an "Adversarial Review (optional)" subsection). One refinement during pressure-testing (`../plans/eval/2026-06-13-roast-eval.md`): the judge-aggregation severity rule was tightened from a loose "majority/median of confirming judges" to "**confirmed = ≥2/3; severity = highest among confirming judges (round up); an unconfirmed finding any judge called blocker/major → flag for human**." All five behavior scenarios and three adversarial pressures pass. As noted in the spec, `roast` could not be dogfooded on its own design at build time; this spec received the normal inline self-review.

**2026-06-13 — Dogfood revision (self-roast).** Once built, `roast` was run on this very spec (6 critics → dedup → 3-judge panel). Verdict REVISE; 16/19 findings confirmed. All confirmed findings except F8 (unbounded-cost cap) were adopted — see `../plans/eval/2026-06-13-roast-eval.md` for the full list. Headline fixes: ground via WebSearch/WebFetch instead of the un-spawnable `deep-research` skill (F6); require citations with an UNVERIFIED escalation path and a fail-safe VERDICT schema (F5/F18); dedup before verify (F7); median (not max) severity for the gate (F14); judge-failure re-dispatch + coverage gate so a broken run can't emit a clean PASS (F12/F4); and explicit honesty that an all-Claude panel is "agreement, not independent verification," recommending a non-Claude judge seat (F1/F3). The recommended de-risking spike (measure whether the same-family panel actually decorrelates vs a single judge) remains open for a future eval.
