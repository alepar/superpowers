...
- **The traversal cursor is a query, not a memory** — but a bare label query returns an unordered set and bd has no topological sort, so ordering is *stored*, not inferred: an epic is **eligible for design** iff it carries `sp:needs-design` AND every earlier sibling (lower `sp_order` under the same parent) has a fully-designed subtree (no `sp:needs-design` anywhere below it — a recursive check over `sp:`-labeled children). The next work item is the eligible epic with the deepest, earliest position (depth-first). This preserves the "later siblings read earlier siblings' *complete* specs" guarantee across any resume. No-beads: same rule over the table ids/deps/depth columns.
- **Write ordering & idempotency (crash between the two stores):** run state spans the beads DB and git-committed spec files, which are not transactional. Rules: (1) decomposition creates children idempotently — re-entry first queries existing `sp:`-labeled children and creates only what's missing; (2) a nested brainstorm commits its spec *before* clearing `sp:needs-design`, so the label flip is the single atomic "done" marker; (3) Spike 0 verifies bd writes survive a killed session (Dolt auto-commit may be off — if labels/metadata sit in a working set until an explicit commit, super-plan must issue that commit after each state-changing step).
...
**No-beads mode:** `writing-plans` produces one plan per epic covering that epic's leaf tasks (a mixed epic still gets a plan for its leaves). SDD's plan-file mode is invoked **once per plan, serially, in dependency order** (its documented single-plan contract, run N times — not a new multi-plan mode). Serial plan execution is also what enforces cross-subtree dependencies here: they hold at plan granularity (a consumer's plan simply runs after its producer's), which is coarser than beads' task-level `bd ready` gating but sufficient because plans never interleave. Task ids, deps, and depth live in the spec task tables (see Durable State). This path is accepted extra scope: the user explicitly wants the flow to work without a tracker; it is exercised in Validation, and its known cost (hand-maintained paper state) is acknowledged.
