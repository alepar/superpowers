...
- **`super-auto` — idea to finished code, one invocation.** Sequences `brainstorming` → `super-design` → `super-roast` (design) → `super-code` → `super-roast` (PR) → a fix loop → a final report → `finishing-a-development-branch`, threading four flags through every phase without reimplementing any of them. An optional autonomous mode suppresses further queries once `super-design`'s coverage loop passes, parking escalations and beyond-cap findings for the final report instead of auto-adjudicating them. **Status: never run end to end.** Its `SKILL.md` carries a *Known limitations* section listing eight documented gaps — read it before an unattended run.
