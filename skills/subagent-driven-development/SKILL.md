...
digraph when_to_use {
    ...
    "Subagents available?" -> "subagent-driven-development" [label="yes (default)"];
    "Subagents available?" -> "executing-plans" [label="no - fallback"];
}
```

**vs. Executing Plans (no-subagent fallback):**
...
- **No `Workflow` tool, subagents available → manual ready-driven loop (fallback).** Run the loop yourself:
  1. **Query ready work.** Run `bd ready --exclude-type=epic --label sp:<root-epic-id>` (the epic id the controller was handed) to get the blocker-aware set of currently-available leaf tasks. Both flags are required: `bd ready` includes epic-type containers by default and is repo-global, so without them the loop dispatches implementers against containers and against out-of-tree tasks in a repo with unrelated epics. Confirm scoping/flags with `bd ready --help`; `--json` gives machine-readable output.
  ...
  3. **Implement phase (parallel where feasible).** Read each ready task's stored plan to get its file list. Group tasks by disjoint files and dispatch implementer subagents accordingly (parallel for non-overlapping, serial for overlapping). For each task: implementer (against its beads-issue plan) → spec-compliance review → code-quality review → fix loops until both pass → close the task with `bd close <id>` (confirm with `bd close --help`). This closes leaf tasks only — epics are never closed with `bd close`; they're closed separately via `bd epic close-eligible` (see step 4).
  4. **Refill and epic closure.** Closing tasks unblocks dependents. `bd epic close-eligible` closes only one tree level per call, so after each refill cycle loop it to a fixpoint (repeat until a pass closes nothing; with `--json` the stop signal is output exactly `[]`, not a `count: 0` object). Then re-query ready work and repeat from step 1.
  5. **Finish.** When the root epic is closed, proceed to `superpowers:finishing-a-development-branch`. An empty ready set is not the finish signal — epics stay open after their leaves close, and the ready set can be transiently empty mid-cascade.
...
- **superpowers:executing-plans** - Automatic fallback only when subagents are unavailable
